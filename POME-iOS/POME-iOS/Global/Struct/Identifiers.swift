//
//  Identifiers.swift
//  POME-iOS
//
//  Created by EUNJU on 2022/07/07.
//

import Foundation

struct Identifiers {
    static let WriteSB = "WriteSB"
    static let RemindSB = "RemindSB"
    static let MateSB = "MateSB"
    static let MypageSB = "MypageSB"
    static let SignUpSB = "SignUpSB"
    static let SignInSB = "SignInSB"
    static let AddFriendSB = "AddFriendSB"
    static let AddFriendVC = "AddFriendVC"
    static let SignUpNC = "SignUpNC"
    static let LookbackSB = "LookbackSB"
    static let LookbackCompleteSB = "LookbackCompleteSB"
    static let WriteSelectFeelingSB = "WriteSelectFeelingSB"
    static let AddGoalDateSB = "AddGoalDateSB"
    static let AddGoalContentSB = "AddGoalContentSB"
    static let AddCompleteSB = "AddCompleteSB"
    static let AddRecordSB = "AddRecordSB"
    static let GoalCardCVC = "GoalCardCVC"
    static let HaveNoMateTVC = "HaveNoMateTVC"
    static let MateHeaderCVC = "MateHeaderCVC"
    static let HaveMateTVC = "HaveMateTVC"
    static let RemindGoalTitleTVC = "RemindGoalTitleTVC"
    static let RemindFilterTVC = "RemindFilterTVC"
    static let RemindGoalTVC = "RemindGoalTVC"
    static let RemindHaveNoGoalTVC = "RemindHaveNoGoalTVC"
    static let RemindGoalCategoryCVC = "RemindGoalCategoryCVC"
    static let MarshmellowCVC = "MarshmellowCVC"
    static let GoalStorageTitleCVC = "GoalStorageTitleCVC"
    static let MateEmojiHeaderCVC = "MateEmojiHeaderCVC"
    static let MateEmojiCVC = "MateEmojiCVC"
    static let HaveNoGoalInStorageCVC = "HaveNoGoalInStorageCVC"
}
